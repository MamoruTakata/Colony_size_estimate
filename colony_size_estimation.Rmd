...
mark <- read_excel("Dataset.xlsx", sheet="mark recapture") %>% 
  mutate(total_W = no_of_unmarked_wroker + no_of_marked_workers_stayed_in_introduced_log + no_of_marked_workers_dispersed_from_another_log) %>% 
  as.data.frame(.)
...
summary_mark <- mark %>% 
  ...
  summarize(mean = mean(no_of_unmarked_wroker)) %>% 
  summarize(number = n())
summary_mark
```
